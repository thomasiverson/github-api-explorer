...
export function TopBar() {
  ...
  const checkAuth = useCallback(async (envId: string) => {
  ...
  return (
    <header className="flex flex-col shrink-0 border-b border-border bg-panel overflow-x-hidden">
      {/* Row 1: Branding, connection info, theme toggle */}
      <div className="h-12 flex items-center px-2 sm:px-4 gap-2 sm:gap-4">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 sm:mr-2 hover:opacity-80 transition-opacity shrink-0">
          <svg width="28" height="28" viewBox="0 0 16 16" fill="currentColor" className="text-text-primary">
            <path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" />
          </svg>
          <span className="font-semibold text-base text-text-primary hidden md:inline">GitHub API Explorer</span>
          <span className="text-xs text-text-secondary hidden xl:inline">Explore, test, and compare GitHub APIs</span>
        </Link>
        ...
        <select
          className="bg-surface border border-border rounded-md px-2 sm:px-3 py-1 text-sm text-text-primary
                     focus:outline-none focus:ring-1 focus:ring-accent min-w-0 w-40 sm:w-auto sm:min-w-[180px]"
          value={activeEnv?.id || ''}
      ...
      {/* Row 2: Navigation */}
      <nav className="h-10 flex items-center justify-start lg:justify-center px-2 sm:px-4 gap-1 border-t border-border overflow-x-auto">
        <Link href="/" className={navLinkClass('/')}>API Explorer</Link>
        <Link href="/assessment" className={navLinkClass('/assessment')}>Assessment</Link>
        <Link href="/history" className={navLinkClass('/history')}>History</Link>
        <Link href="/collections" className={navLinkClass('/collections')}>Collections</Link>
        <Link href="/compare" className={navLinkClass('/compare')}>Compare</Link>
        <Link href="/templates" className={navLinkClass('/templates')}>Templates</Link>
        <Link href="/webhooks" className={navLinkClass('/webhooks')}>Webhooks</Link>
        <Link href="/graphql" className={navLinkClass('/graphql')}>GraphQL</Link>
        <Link href="/graphql/batch-orgs" className={navLinkClass('/graphql/batch-orgs')}>Batch Create Orgs</Link>
